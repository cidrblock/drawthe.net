...
export function showAlert(container: HTMLElement, type: AlertType, message: string, timeoutMs = 4000): void {
  ...
  alertEl.innerHTML = `${message}<button type="button" class="close" aria-label="Close">&times;</button>`;
  alertEl.querySelector(".close")?.addEventListener("click", () => alertEl.remove());
